...
export default class SessionStorage<I extends Item, B extends Basket> implements Store<I> {
  ...
  public add(value: I): boolean {
    try {
      sessionStorage.setItem('basket', JSON.stringify(this.basketActions.addItem(value, this.getBasket())));
      return true;
    } catch (error) {
  ...
  public increment(mpn: string, qty: number): boolean {
    try {
      sessionStorage.setItem('basket', JSON.stringify(this.basketActions.incrementItem(mpn, qty, this.getBasket())));
      return true;
    } catch (error) {
  ...
  public decrement(mpn: string, qty: number): boolean {
    try {
      sessionStorage.setItem('basket', JSON.stringify(this.basketActions.decrementItem(mpn, qty, this.getBasket())));
      return true;
    } catch (error) {
      throw new Error('increment: unable to update item quantity ' + error);
    }
  ...
  public remove(mpn: string): boolean {
    try {
      sessionStorage.setItem('basket', JSON.stringify(this.basketActions.removeItem(mpn, this.getBasket())));
      return true;
    } catch (error) {
  ...
  private getBasket(): B {
    try {
      return JSON.parse(sessionStorage.getItem('basket') || JSON.stringify(this.emptyBasket));
    } catch (error) {
